...
export function StepperFormDemo({ onComplete }) {
  ...
  /* --------------------------
      SUBMIT (TRIGGERED ONLY BY COMPLETE)
  -------------------------- */
  ...
  return (
    <form onSubmit={onSubmit} className="w-[80%] mx-auto">
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4">
              ...
              {form.paymentMethod === "card" && (
                <div className="grid grid-cols-3 gap-4">
                  <Input placeholder="XXX XXX XXX XXX" value={form.cardNumber} onChange={(e) => update("cardNumber", e.target.value)} />
              ...
              {form.paymentMethod === "cash" && (
                <p className="text-sm text-muted-foreground">You will pay in cash upon delivery or visit.</p>
              )}
            </div>
          )}
      ...
      <div className="mt-6 flex justify-between">
        ...
        {step === steps.length - 1 ? (
          <Button type="submit" className="cursor-pointer">Complete</Button>
        ) : (
