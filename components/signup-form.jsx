...
export function SignupForm({ onSignupSuccess, className, ...props }) {
  ...
  const handleSignUp = async () => {
    ...
    // Immediately store pending flag (so page will show alert on next render)
    localStorage.setItem(PENDING_VERIFICATION_KEY, "true");
    localStorage.setItem(`${PENDING_VERIFICATION_KEY}_email`, email);
    ...
    try {
      ...
      // Notify parent page to switch to verification alert
      if (onSignupSuccess) onSignupSuccess(user);
    } catch (err) {
      // Signup failed – remove pending flag
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        toast.error("An account already exists with the same email address but different sign-in method.");
      } else {
